  ...
  return (
    <Dialog onOpenChange={handleCloseDialog} open={open}>
      <DialogContent className="sm:max-w-[500px]">
        ...
        {shopsHaveBook.length > 0 && (
          <div className="mt-4 flex justify-end">
            <button
              className="rounded-md bg-blue-600 px-4 py-2 text-white hover:bg-blue-700"
              onClick={() => {
                if (selectedShopId) {
                  console.log("Shop được chọn:", selectedShopId);
                  // Bạn có thể thêm xử lý khác như gọi API hoặc setState
                  setOpen(false); // hoặc đóng dialog nếu muốn
                } else {
                  alert("Vui lòng chọn một cửa hàng trước khi xác nhận.");
                }
              }}
            >
              Xác nhận
